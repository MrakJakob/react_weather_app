...
export const Card = ({data}) => {
    ...
    return (
        <Box flex="1" borderWidth="2px" borderRadius="md" borderColor="gray.200" h="full" p={4} m="4" bg="rgba(255, 255, 255, 0.3)" borderRadius="10pt" maxWidth={300}>
            <div className="weatherBox">
                <div className='day'>{day(data.dt)}</div>
              <div className="weatherForecast">{data.weather[0].description}</div>
              <div className="iconForecast">{renderIcon(data.weather[0].description)}</div>
              <div className="temperatureForecast">{Math.floor(data.main.temp)}°C</div>
            </div>
        </Box>
    )
}